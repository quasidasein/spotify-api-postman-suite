...
* **[Boundary] GET Artist's Albums - Out of Bounds Offset**
  * **Эндпоинт:** `GET {{base_url}}/v1/artists/6TsAG8Ve1icEC8ydeHm3C8/albums?offset=100000`
  * **Ожидаемый статус:** `200 OK`
  * **Проверки:**
    * Отказоустойчивость бэкенда: статус `200 OK` без падения сервиса при смещении за пределы базы данных.
    * Возврат пустого массива данных (`items: []`).
    * Ссылка на следующую страницу отключена (`next === null`).
    * Наличие ссылки возврата на предыдущую страницу (`previous` является URL-строкой).
    * Эхо-валидация смещения: сервер явно возвращает запрошенное значение (`offset === 100000`).
